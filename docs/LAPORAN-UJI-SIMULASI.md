...
## 8. Tahap 10 (7 September 2026) — perangkat, keandalan aplikasi, panel admin, ukuran APK
Tim: Engineer Device & Media, Engineer Aplikasi Pelanggan, Designer Panel Admin, Engineer Platform & Data, QC.

| Permintaan komisaris | Hasil |
|---|---|
| 1. Telepon, chat, notifikasi suara, loudspeaker, fitur perangkat | Modul panggilan ditulis ulang: batas waktu langganan (dulu bisa menggantung selamanya), deteksi penerima tidak aktif (12 dtk), "tidak dijawab" 40/45 dtk, anti panggilan ganda, izin mikrofon berbahasa Indonesia + tombol buka Pengaturan, watchdog sambungan ulang, dan **perbaikan bug "sedang sibuk" abadi** (panggilan berikutnya selalu ditolak sebelum pengguna menutup layar). Nada dering + getar, bunyi order baru (driver) & pesan chat baru (aset WAV 21 KB buatan sendiri). **Batas jujur**: pemindahan earpiece↔loudspeaker belum bisa (butuh modul audio native), tombolnya tidak lagi berpura-pura menyala; notifikasi saat aplikasi tertutup belum ada (butuh `expo-notifications` + FCM) |
| 2. Menu pelanggan sering blank; sakelar layanan admin terasa delay | Akar masalah blank: `Entrance` memakai animasi layout Reanimated (`entering`) yang di Android release kadang tidak dijalankan sehingga elemen tetap `opacity: 0` selamanya → diganti animasi berbasis mount + jaring pengaman timer yang memaksa terlihat. Sakelar layanan: cache 5 menit → 60 detik, muat ulang saat aplikasi aktif & beranda difokuskan, **langganan realtime `app_settings`** (migrasi 0028 mendaftarkan tabelnya ke publication) |
| 3. Notifikasi diketuk tidak ada respons | Semua notifikasi kini punya tujuan (`ticket_id` → tiket CS, `order_id` → detail pesanan, travel, AntarPay, merchant/promo); tanpa tujuan → bottom-sheet detail. Badge "Ketuk untuk membuka …". QC menemukan 1 notifikasi DB tanpa kunci tujuan (pembatalan jadwal travel) → diperbaiki di migrasi 0029 |
| 4. Audit font & shape, tata letak tombol, kota pakai dropdown | Panel admin: aksi baris (dulu 7 tombol membungkus 2–3 baris) → maks 2 aksi + menu ⋮; tinggi baris seragam 60 px, kolom aksi lebar tetap; token tipografi/ikon/radius dipakai konsisten di 24 layar + Portal Eksekutif; pemilih kota jadi dropdown (`AdminSelect`) di 9 layar admin dan `Dropdown` di AntarTravel & AntarSend; warna tombol simpan dikembalikan ke warna merek |
| 5. Pertimbangan ukuran APK | `docs/UKURAN-APK-DAN-DATA.md`: **terukur** — impor ikon per-keluarga menghapus 22 font mubazir (aset 4,00 MB → 496 KB) dan bundel JS 3,80 MB → 3,56 MB (dist 3 aplikasi 26 MB → 15 MB); ABI split arm (perkiraan −15…20 MB APK); dependensi tak terpakai dihapus. Kesimpulan penting: **memindahkan data ke database tidak mengecilkan APK** karena gambar promo/merchant/katalog sudah dari server — yang membesarkan APK adalah kode native (WebRTC, RN, Hermes) |
| 6. Uji semua fitur + tata ulang database | Simulasi diperluas jadi **34 skenario (S0–S33)**, semuanya LOLOS (116 baris pemeriksaan, rollback penuh, data live terbukti tidak berubah). Database: realtime `app_settings`, **perbaikan RLS `order_messages` (admin tidak bisa membalas chat pesanan)**, 52 indeks foreign key, 5 fungsi diberi `search_path`, hak akses anon dicabut dari 2 fungsi; sisa temuan advisor didokumentasikan |

**Bug nyata yang ditemukan & diperbaiki hari ini**: (a) panggilan bisa menggantung selamanya bila langganan realtime tidak pernah SUBSCRIBED; (b) status "ended" membuat setiap panggilan masuk berikutnya dijawab "sedang sibuk"; (c) tombol speaker tidak melakukan apa pun (API tidak ada); (d) `Entrance` meninggalkan konten tak terlihat (layar blank); (e) admin tidak bisa mengirim pesan di chat pesanan (RLS) — 0028; (f) notifikasi pembatalan jadwal travel tanpa kunci tujuan — 0029; (g) pesan galat jaringan mentah (`java.net.UnknownHostException`) kini jadi "Tidak ada koneksi internet…" di **semua** toast.
