// Banner & catatan kecil saat AntarPay dinonaktifkan dari Panel Admin (migrasi 0088).
// Dipakai di tab AntarPay, top up, gateway, pencairan mitra, dan pilihan metode bayar.
import React from 'react';
import { View, Text, StyleSheet } from 'react-native';
import Ionicons from '@expo/vector-icons/Ionicons';
import { ANTARPAY_OFF_TEXT } from '@/hooks/useAppSettings';
import { colors, font, radius } from '@/lib/theme';

/** Banner penuh (kartu kuning) — untuk layar dompet/top up/pencairan. */
export function AntarPayOffBanner({ text = ANTARPAY_OFF_TEXT, style }: { text?: string; style?: object }) {
  return (
    <View style={[s.banner, style]}>
      <View style={s.icon}><Ionicons name="pause-circle" size={20} color="#fff" /></View>
      <View style={{ flex: 1, minWidth: 0 }}>
        <Text style={s.title}>AntarPay sementara nonaktif</Text>
...
export function AntarPayOffNote({ text = 'AntarPay sementara nonaktif — pembayaran hanya tunai.' }: { text?: string }) {
  return (
    <View style={s.note}>
