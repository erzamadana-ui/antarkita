...
import { AdminPage, Panel, DataTable, Toolbar, FilterBar, StatCard, Pill, SoftChip, AdminSelect, Grid, Col, adminFont as font, adminTone, adminSpace, adminRadius, adminIcon, TONE, type ToneKey } from '@/components/admin';
...
export default function AdminEconomicsReport() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try { setData(await rpc<SkemaReport>('admin_exec_report_v2', { p_from: range.from, p_to: range.to, p_filters: filters })); setErr(null); }
  ...
  return (
    <AdminPage title="Laporan Skema Bisnis" subtitle="Unit economics dari buku besar order: GMV bersih, take rate bersih, contribution, EBITDA kota, dan gerbang scale-up" onRefresh={load}
