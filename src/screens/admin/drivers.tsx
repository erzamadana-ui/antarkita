...
import {
  AdminPage, DataTable, Toolbar, ReasonPrompt, StatCard, Grid, Pill, StatusPill, RowActions, DeletePartnerDialog, IconAction, adminFont as font, adminTone, adminSpace, adminTable, adminIcon,
} from '@/components/admin';
import { Row, Button, toast } from '@/components/ui';
import { rpc, supabase } from '@/lib/supabase';
import { signedUrl } from '@/lib/upload';
import { colors } from '@/lib/theme';
import { phoneMasked, vehicleClassLabel, vehicleTypeLabel } from '@/lib/format';
...
export default function AdminDrivers() {
  ...
  const [ask, setAsk] = useState<{ id: string; status: ApprovalStatus; name: string; docs?: boolean } | null>(null);
  ...
  /** Minta dokumen ulang: status dikembalikan ke "menunggu" dengan catatan yang dibaca driver di aplikasi Mitra. */
  const askDocs = (id: string, name: string) => setAsk({ id, status: 'pending', name, docs: true });
  const requestDocs = async (id: string, reason: string) => {
    try { await rpc('admin_set_driver_status', { p_driver: id, p_status: 'pending', p_reason: `Dokumen perlu diunggah ulang: ${reason}` }); toast.success('Permintaan dokumen ulang dikirim & tercatat di log'); setAsk(null); load(); }
    catch (e) { toast.error((e as Error).message); }
  };
  ...
  return (
    <AdminPage title="Mitra Driver" subtitle={`${rows.length} terdaftar · ${count('pending')} menunggu verifikasi · nomor pribadi tersamar`} onRefresh={load}>
      <ReasonPrompt visible={!!ask}
        title={ask?.docs ? `Minta dokumen ulang dari ${ask?.name}` : ask?.status === 'suspended' ? `Tangguhkan ${ask?.name}?` : `Tolak ${ask?.name}?`}
        subtitle={ask?.docs ? 'Tulis dokumen apa yang harus diunggah ulang. Status kembali ke "menunggu" dan catatan ini tampil di aplikasi Mitra.' : 'Alasan wajib — tersimpan di Log Aktivitas dan ditampilkan ke driver.'}
        quick={ask?.docs ? ['Foto KTP buram / terpotong', 'Foto SIM tidak terbaca', 'Foto kendaraan tidak jelas', 'STNK kedaluwarsa'] : undefined}
        color={ask?.docs ? colors.warning : colors.danger}
        onCancel={() => setAsk(null)}
        onSubmit={(r) => (ask?.docs ? requestDocs(ask.id, r) : setStatus(ask!.id, ask!.status, r))}
        confirmLabel={ask?.docs ? 'Minta dokumen ulang' : ask?.status === 'suspended' ? 'Tangguhkan' : 'Tolak'} />
      ...
      <DataTable rows={pg.rows as unknown as Record<string, unknown>[]} emptyText="Tidak ada driver pada filter ini" emptyIcon="bicycle-outline" columns={[
        {
          key: 'name', label: 'Driver', width: 200, render: (r) => {
            const d = r as unknown as Row_;
            return (
              <View style={{ minWidth: 0, alignSelf: 'stretch' }}>
                <Trunc style={font.bodyStrong} title={d.profile?.full_name ?? ''}>{d.profile?.full_name ?? '—'}</Trunc>
                <Trunc style={font.tiny}>{phoneMasked(d.profile?.phone)} · bergabung {fmtDate(d.created_at, false)}</Trunc>
              </View>
            );
          },
        },
        {
          key: 'vehicle', label: 'Kendaraan', width: 200, render: (r) => {
            const d = r as unknown as Row_;
            ...
            return (
              <View style={{ minWidth: 0, alignSelf: 'stretch' }}>
                <Trunc style={font.bodyStrong} title={head}>{head}{fuel ? ` · ${fuel}` : ''}</Trunc>
                <Trunc style={font.tiny}>{vehicleTypeLabel[d.vehicle_type]} · {d.vehicle_plate}{d.vehicle_year ? ` · ${d.vehicle_year}` : ''} · {d.vehicle_class ? vehicleClassLabel[d.vehicle_class] ?? d.vehicle_class : '—'}</Trunc>
              </View>
            );
          },
        },
        {
          key: 'docs', label: 'Dokumen', width: 132, render: (r) => {
            const d = r as unknown as Row_;
            return (
              <View style={{ gap: 4, minWidth: 0, alignSelf: 'stretch' }}>
                <Trunc style={font.tiny} title={`SIM ${d.docs?.license_number ?? '-'} · NIK ${d.docs?.id_card_number ?? '-'}`}>SIM {d.docs?.license_number ?? '-'}</Trunc>
                <Row gap={6}>
        ...
        {
          key: 'rating_avg', label: 'Performa', width: 100, render: (r) => {
            const d = r as unknown as Row_;
            return (
        ...
        {
          key: 'status', label: 'Status', width: 116, render: (r) => {
            const d = r as unknown as Row_;
            return (
              <View style={{ gap: 3, minWidth: 0, alignSelf: 'stretch' }}>
                <StatusPill status={d.status} />
                {d.status_reason && d.status !== 'approved' ? <Trunc style={font.tiny} title={d.status_reason} lines={2}>{d.status_reason}</Trunc> : null}
              </View>
            );
          },
        },
          ...
          key: 'actions', label: 'Aksi', width: adminTable.actionsWideW, align: 'right', render: (r) => {
            ...
            return (
              <RowActions
                ...
                menu={[
                  { key: 'docs', label: 'Minta dokumen ulang…', icon: 'cloud-upload-outline', color: adminTone.amber, hint: 'status kembali ke menunggu', onPress: () => askDocs(d.id, name) },
      ...
      <WideTableHint />
      <Pager p={pg} noun="driver" hint="maks. 300 pengajuan terbaru dimuat" />
