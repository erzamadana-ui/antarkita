...
import { getSecrets, type GatewaySecrets } from "../config.ts";
...
/**
 * Adaptor untuk (provider, env) — KETAT per env (T1): tidak pernah jatuh ke kunci env lain. Env transaksi yang sudah
 * ada diambil dari payments.env (paymentEnv), bukan dari setting saat ini.
 */
export async function providerFor(deps: Deps, name: ProviderName, env: ProviderEnv): Promise<PaymentProvider | null> {
  if (name === "simulated") return new SimulatedProvider(env);
  const s = await getSecrets(deps.db, name, env, deps.env);
  return buildProvider(name, s, deps, env);
}

/** Rahasia disbursement untuk SATU env: provider 'finpay_disbursement' bila ada, selain itu kredensial 'finpay'. */
